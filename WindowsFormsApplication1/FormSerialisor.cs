      ...
      foreach (Control childCtrl in c.Controls) {
        if (!(childCtrl is Label)) {
          ...
          if (childCtrl is TextBox) {
            ...
            xmlSerialisedForm.WriteElementString("Kolorek", ((TextBox)childCtrl).BackColor.ToArgb().ToString());
          ...
          if (ctrlToSet.GetType().ToString() == controlType) {
            ...
            switch (controlType) {
              case "System.Windows.Forms.TextBox":
                ...
                ((System.Windows.Forms.TextBox)ctrlToSet).BackColor =     Color.FromArgb(Convert.ToInt32(n["Kolorek"].InnerText));
                break;
